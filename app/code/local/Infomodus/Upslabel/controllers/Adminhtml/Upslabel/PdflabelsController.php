...
class Infomodus_Upslabel_Adminhtml_Upslabel_PdflabelsController extends Mage_Core_Controller_Front_Action
{
            ...
            foreach ($colls AS $collection) {
                if (file_exists($img_path . $collection->getLabelname()) && filesize($img_path . $collection->getLabelname()) > 512) {
                    if ($collection->getTypePrint() == "GIF") {
                        $pdf->pages[] = self::_setLabelToPage($img_path . $collection->getLabelname(), $storeId);
    ...
    static public function create($order_ids, $type, $ptype)
    {
        ...
        foreach ($order_ids as $order_id) {
            ...
            if (Mage::getStoreConfig('upslabel/printing/bulk_printing_all', $storeId) == 1) {
                    ...
                    if (($collection->getOrderId() == $order_id && $ptype == "order") || ($collection->getShipmentId() == $order_id && $ptype != "order")) {
                        if (file_exists($img_path . $collection->getLabelname()) && filesize($img_path . $collection->getLabelname()) > 1024) {
                            if ($collection->getTypePrint() == "GIF") {
                                ...
                                $ip = trim(Mage::getStoreConfig('upslabel/printing/automatic_printing_ip', $storeId));
                                $port = trim(Mage::getStoreConfig('upslabel/printing/automatic_printing_port', $storeId));
                                if (strlen($ip) > 0 && strlen($port) > 0 && Mage::getStoreConfig('upslabel/printing/printer', $storeId) != 'GIF') {
    ...
    private static function _setLabelToPage($label, $storeId = NULL)
    {
        ...
        $xSize = imagesx($image);
        $ySize = imagesy($image);
        /*if (Mage::getStoreConfig('upslabel/printing/printer', $storeId) == "GIF") {*/
        if (Mage::getStoreConfig('upslabel/printing/papersize', $storeId) != "AC") {
            if (Mage::getStoreConfig('upslabel/printing/papersize', $storeId) == "A4") {
                if ($xSize > 595) {
            ...
            $ySize = Mage::getStoreConfig('upslabel/printing/custom_width', $storeId) * ($ySize / $xSize);
            $xSize = Mage::getStoreConfig('upslabel/printing/custom_width', $storeId);
            $page = new Zend_Pdf_Page($xSize, $ySize);
        }
