...
class Infomodus_Upslabel_Model_Ups
{
    ...
    function getShip()
    {
        ...
        $this->customerContext = str_replace('&', '&amp;', strtolower(Mage::app()->getStore( /*multistore*/
            $this->storeId /*multistore*/)->getName()));
        $validate = Mage::getStoreConfig('upslabel/shipping/validate' /*multistore*/, $this->storeId /*multistore*/) == 1 ? 'validate' : 'nonvalidate';
...
<ShipmentConfirmRequest xml:lang=\"en-US\">
  ...
  <LabelSpecification>
    <LabelPrintMethod>
      <Code>" . Mage::getStoreConfig('upslabel/printing/printer', $this->storeId) . "</Code>
    </LabelPrintMethod>
    ";
        if (Mage::getStoreConfig('upslabel/printing/printer', $this->storeId) != "GIF") {
            $data .= "<LabelStockSize>
                <Height>4</Height>
                <Width>" . Mage::getStoreConfig('upslabel/printing/termal_width', $this->storeId) . "</Width>
            </LabelStockSize>";
        }
        $data .= "
  ...
  <Shipment>";
        if (Mage::getStoreConfig('upslabel/ratepayment/negotiatedratesindicator', $this->storeId) == 1) {
            $data .= "
   <RateInformation>
    ...
    ";
      ...
      <Description>" . $this->serviceDescription . "</Description>
        ...
        if ($this->shiptoCountryCode != $this->shipfromCountryCode || ($this->shiptoCountryCode == $this->shipfromCountryCode && $this->shiptoCountryCode != 'US' && $this->shiptoCountryCode != 'PR')) {
            ...
            if (Mage::getStoreConfig('upslabel/packaging/packagingreferencebarcode', $this->storeId) == 1) {
		...
		<Value>" . $this->packages[0]['packagingreferencenumbervalue'] . "</Value>
            ...
            if (isset($this->packages[0]['packagingreferencenumbercode2'])) {
                $data .= "<ReferenceNumber>";
                if (Mage::getStoreConfig('upslabel/packaging/packagingreferencebarcode2', $this->storeId) == 1) {
                    $data .= "<BarCodeIndicator></BarCodeIndicator>";
                }
...
<Height>" . $pv['height'] . "</Height>";
                    ...
                    $data .= "<Length>" . Mage::getStoreConfig('upslabel/dimansion_' . $pv['dimansion_id'] . '/length', $this->storeId) . "</Length>
<Width>" . Mage::getStoreConfig('upslabel/dimansion_' . $pv['dimansion_id'] . '/width', $this->storeId) . "</Width>
<Height>" . Mage::getStoreConfig('upslabel/dimansion_' . $pv['dimansion_id'] . '/height', $this->storeId) . "</Height>";
                }
                $data .= "</Dimensions>";
            }
            $data .= "<PackageWeight>
...
<br /><br />
<p class=MsoNormal>' . $this->trackingNumber . ' <span class=GramE>' . $this->packages[0]['packagingreferencenumbervalue'] . ' ' . round($this->codMonetaryValue, 2) . '</span> ' . Mage::app()->getLocale()->currency(Mage::app()->getStore( /*multistore*/
                                $this->storeId /*multistore*/)->getCurrentCurrencyCode())->getSymbol() . ' </p>
<br /><br />
