...
class Infomodus_Upslabel_Model_Observer
{
    ...
    public function frontorderplace(Varien_Event_Observer $event)
    {
        ...
        if (Mage::getStoreConfig('upslabel/frontend_autocreate_label/frontend_order_autocreate_label_enable' /*multistore*/, $order->getStoreId() /*multistore*/) == 1) {
            /*multistore*/
            $storeId = NULL;
            ...
            /*multistore*/
            $shippingActiveMethods = trim(Mage::getStoreConfig('upslabel/frontend_autocreate_label/apply_to', $storeId), " ,");
            $shippingActiveMethods = strlen($shippingActiveMethods) > 0 ? explode(",", $shippingActiveMethods) : array();
            $orderStatuses = explode(",", trim(Mage::getStoreConfig('upslabel/frontend_autocreate_label/orderstatus', $storeId), " ,"));
            if (((isset($shippingActiveMethods) && count($shippingActiveMethods) > 0 && in_array($order->getShippingMethod(), $shippingActiveMethods)) || strpos($order->getShippingMethod(), "ups_") === 0)
                ...
                if (count($colls) == 0) {
                    ...
                    $lbl = $controller->setParams($lbl, $controller->defConfParams, $controller->defParams /*multistore*/, $storeId /*multistore*/);
    ...
    public function callback_upsstatus($value, $row, $column, $isExport)
    {
        ...
        if ($item->getStatustext()) {
            ...
            $shippingActiveMethods = trim(Mage::getStoreConfig('upslabel/frontend_autocreate_label/apply_to', $storeId), " ,");
            $shippingActiveMethods = strlen($shippingActiveMethods) > 0 ? explode(",", $shippingActiveMethods) : array();
            if (Mage::getStoreConfig('upslabel/shipping/shipping_method_native', $storeId) == 1) {
                $modelConformity = Mage::getModel("upslabel/conformity")->getCollection()->addFieldToFilter('method_id', $order->getShippingMethod())->addFieldToFilter('store_id', /*multistore*/
    ...
    public function _orderUpsStatusFilter($collection, $column, $type = "shipment", $id = "order_id")
    {
        ...
        if ($is_need_filter == true) {
            ...
            if ($value == "pending") {
                ...
                if (Mage::getStoreConfig('upslabel/shipping/shipping_method_native', $storeId) == 1) {
                    $modelConformity = Mage::getModel("upslabel/conformity")->getCollection()->addFieldToFilter('store_id', /*multistore*/
                        $storeId ? $storeId : /*multistore*/
                            1);
                ...
                //$modelConformity = Mage::getModel("upslabel/conformity")->getCollection()->addFieldToFilter('store_id', /*multistore*/ $storeId ? $storeId : /*multistore*/1);
