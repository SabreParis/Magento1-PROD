 ...
 * @package Ayaline_Billboard
 */
class Ayaline_Billboard_Block_Adminhtml_Catalog_Product_Edit_Tab_Billboard extends Mage_Adminhtml_Block_Widget_Grid
{

    public function __construct()
    {
        parent::__construct();
        $this->setId('ayaline_billboard_product');
    ...
    {
        if ($column->getId() == 'billboard_in_product') {
    ...
    protected function _prepareCollection()
    {
        if ($this->getProduct()->getId()) {
            $this->setDefaultFilter(array('billboard_in_product' => 1));
        }
        ...
        $collection->getSelect()->joinLeft(
            array('abbp' => $collection->getTable('ayalinebillboard/billboard_product')),
            "main_table.billboard_id = abbp.billboard_id AND abbp.product_id = '" . (int)$this->getRequest()->getParam('id', 0) . "'",
            array('position' => 'abbp.position')
        );
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('billboard_in_product', array(
            'header_css_class' => 'a-center',
            'type'             => 'checkbox',
            'name'             => 'billboard_in_product',
    ...
    {
        return $this->getUrl('*/ayaline_billboard_product/billboardGrid', array('_current' => true));
    }
    ...
    {
        $billboardsPosition = Mage::getResourceSingleton('ayalinebillboard/billboard')->getBillboardProductPosition($this->getProduct());

        return array_keys($billboardsPosition);
    }
    ...
    public function getSelectedBillboards()
    {
        $billboards = array();
        $billboardsPosition = Mage::getResourceSingleton('ayalinebillboard/billboard')->getBillboardProductPosition($this->getProduct());
        foreach ($billboardsPosition as $_billboardId => $_position) {
